...
//			TestWindows sendwin = new TestWindows("推送消息");
		} catch (Exception ex) {
			ex.printStackTrace();
			System.exit(1);
		}
	...
	private void start() {
		try {
			...
			// "/Volumes/data/my-private/androidpn-server-0.5.0/";
			...
			File confDir = new File(directory.getCanonicalPath()
					+ File.separator + "conf");
			...
			File libDir = new File(directory.getCanonicalPath()
					+ File.separator + "lib");
			...
			Class containerClass = loader
					.loadClass("android.apn.androidpn.server.xmpp.XmppServer");
